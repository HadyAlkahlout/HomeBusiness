...
import com.nurbk.ps.homebusness.ui.viewmodel.home.AllSpecialStoreViewModel
...
class AllSpacialStoryFragment : Fragment() {


    private lateinit var mBinder: FragmentAllBinding
    private val viewModel by lazy {
        ViewModelProvider(this)[AllSpecialStoreViewModel::class.java]
    }

    private val newProductAdapter =
        HomeStoreAdapter(ArrayList(), 3, object : HomeStoreAdapter.OnClickListener {
            override fun onClickItem(data: SpecialMarket) {
                val bundle = Bundle()
                bundle.putString(Constant.DETAILS_STORE, data.id.toString())
                findNavController().navigate(
                    R.id.action_allSpacialStoryFragment_to_storeDetailsFragment,
                    bundle
                )
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        txtTitleAll.text = getString(R.string.spacial_stores)
        ...
        viewModel.getDataSpecialStore()

        viewModel.dataSpecialStoreViewModel.observe(viewLifecycleOwner, Observer {
            when (it) {
                is Resource.Success -> {
